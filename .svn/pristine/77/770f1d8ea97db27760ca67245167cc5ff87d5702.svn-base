             ...
             for (var i = 0; i < dondel.length; i++) {
                 ...
                 dondel[i].onclick = function () {
                     ...
                     layer.open({
                         ...
                         btn: ['确定', '取消'],
                         yes: function () {
                             if (doninputs[m].checked == true) {
                                 //$("div.Rtop0").eq(this.index).remove();
                                 var name = $(".getfriendnames").eq(m).html();
                                 var ID = $(".messageID").eq(m).html();
         ...
         <div id="R-mess0" class="RM-messclass" style="display:block;">
             <%
                ...
                if (countints > 3)
                    countints = 3;
                     ...
                     {%>
              <div class="Rtop0">
                    <label class="messageID" style="display:none"><%=content[i].ID %></label>
                <table>
          ...
          <div id="friend_id" style="display:none;">
            <%
                ...
                {%>
               <div class="Rtop0ss" style="display:none;">
                   <%
                       int fg = contents[i].list.Count;
                       if (fg > 2)
                       {
                           fg = 2;
                       }
              ...
              <div class="Rtop0s" style="display:block">
                  <label class="messageIDS" style="display:none"><%=contents[i].list[j].IDS %></label>
                <table>
                 <tr><td style="width:70px;">
                     <img src="../../image/imgs/<%=contents[i].list[j].urls %>" style="width:54px;height:54px;"/></td>
                 <td style="width:10px; padding-bottom:18px;">
                         <input type="checkbox" class="CheckBoxclass2" />
                     </td>
                  <td style="width:660px;"><Label  class="getfriendnames"><%=contents[i].list[j].names %></Label>&nbsp;&nbsp;<Label ><%=contents[i].list[j].times%></Label><br /><%=contents[i].list[j].talks %><br /></td>
