        ...
        else
        {
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    var dat = '<div class="evInPiec sorPiec"><span class="linhead"><img src="../../image/noavatar_small.gif"/><em>' + result["evanamehh"] + '</em></span><span class="becRed middevaPie">+1</span><span class="endPiec">' + rjgkf + '</span></div>';
                    $(".okeva").after(dat);
                },
                error: function (result) {
                    alert(result["evanamehh"]);
                    alert("评分失败");
                }
            });
...
window.onload = function () {
    ...
    $("#pmsubmit_btn").next().click(function () {
        $("#pmmessage").value("");
        process(fri_name, et);
    });
    //回复主题
    $(".repkyLogin").click(function () {
        if ($("#usid").text() == 1) {
            ...
            var s = $(this).parent().prev().find("textarea").value();
    ...
    $(".cPublish button").click(function () {
        var title = $(".posting input").value();
        var content = $(".posting textarea").value();
        var id = $("#id").html();
    ...
    //$("inputIn").
    $(".dashbttom").find("div").children("span").eq(1).children("a").click(function () {
        var x = parseInt($(".inputIn").val());
        //alert(x);
        //alert($(".userSgle").eq(x).scrollTop());
        $("html,body").animate({ scrollTop: $(".userSgle").eq(x - 1).offset().top }, 1000);//1000是ms,也可以用slow代替
    });
    $(".inputIn").focus(function (event) {
    ...
    var plg = parseInt($(".selfCon .pinkbg").height());
    var wlg = parseInt($(".selfCon .whitebg").height());
    if (plg < wlg) {
        $(".selfCon .pinkbg").css("height", wlg + 60 + "px");
    }
    else {
...
function scroll() {
    if (parseInt($("html,body").scrollTop()) - 0 > 0) {
...
var scrol = window.setInterval("scroll()", 50);
